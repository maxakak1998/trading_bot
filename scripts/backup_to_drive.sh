...
create_model_metadata() {
    ...
    # Tự động detect model identifier từ thư mục models
    local model_identifier="unknown"
    local model_name="XGBoostRegressor"  # Default FreqAI model
    
    # Tìm model identifier từ cấu trúc thư mục (e.g., freqai-xgboost-v2-stationary)
    if [ -d "${USER_DATA_DIR}/models" ]; then
        # Lấy tên thư mục đầu tiên trong models/ (thường là model identifier)
        model_identifier=$(ls -1 "${USER_DATA_DIR}/models" 2>/dev/null | head -1)
        if [ -z "$model_identifier" ]; then
            model_identifier="empty"
        fi
    fi
    
    # Lấy model_name từ config.json nếu có
    if [ -f "${USER_DATA_DIR}/config.json" ]; then
        local config_model=$(jq -r '.freqai.model_training_parameters.model_type // empty' "${USER_DATA_DIR}/config.json" 2>/dev/null)
        if [ -n "$config_model" ] && [ "$config_model" != "null" ]; then
            model_name="$config_model"
        fi
    fi
    
    # Đếm số sub-models đã train
    local trained_models=0
    if [ -d "${USER_DATA_DIR}/models/${model_identifier}" ]; then
        trained_models=$(find "${USER_DATA_DIR}/models/${model_identifier}" -type d -name "sub-train-*" 2>/dev/null | wc -l | tr -d ' ')
    fi
    
    # Tạo metadata với thông tin đầy đủ hơn
    cat > "$metadata_file" <<EOF
{
    ...
    "trained_submodels": ${trained_models},
...
EOF
    ...
    log "  - Trained Submodels: ${trained_models}"
}
